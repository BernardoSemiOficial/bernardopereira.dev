export enum LabelsSocialNetworks {
  VISIT_MY_GITHUB = 'acesse meu github para ver meus projetos',
  VISIT_MY_LINKEDIN = 'acesse meu linkedin para me acompanhar nas redes sociais',
  VISIT_JSM = 'acesse o site da Juntos Somos Mais e descubra quem ela é',
}

export enum LabelsCompanies {
  VISIT_JSM = 'acesse o site da Juntos Somos Mais e descubra quem ela é',
  VISIT_FIAP = 'acesse o site da FIAP e descubra quem ela é',
  VISIT_ETEC_PQ_BELEM = 'acesse o site da ETEC Parque Belém e descubra quem ela é',
}

export enum LabelsTitles {
  LOGO_IMAGE_TITLE = 'Foto de perfil do Bernardo Pereira',
  UNDER_CONSTRUCTION = 'Essa página está construção',
}
